...
sync.reload = function (bs) {
    ...
    return function (data) {
        ...
        if (data.basename && data.ext) {
            ...
            var elems   = sync.getMatches(domData.elems, data.basename, domData.attr, data);
 ...
 * @returns {Array}
 */
sync.getMatches = function (elems, url, attr, data) {
    ...
    for (var i = 0, len = elems.length; i < len; i += 1) {
        var actualUrl = elems[i][attr];

        // has query string
        if (actualUrl.indexOf('?')) {
            actualUrl = actualUrl.split('?')[0];
        }

        var commonPrefix = sharedStart([
            reverseString(actualUrl),
            reverseString(data.path)
        ]);
